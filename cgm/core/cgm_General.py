...
_d_KWARG_stopAtStep = {
    "kw": "stopAtStep",
    "default": None,
    "help": "Step of a cgmFuncToStopAt",
    "argType": "int/str",
}
...
class cgmFuncCls(object):
    ...
    def __init__(self, *args, **kws):
        ...
        self._b_ExceptionInterupt = (
            True  # Whether to use the cgmFuncCls Exception Interupt or not
        )
        self._str_lastLog = None
        self._int_stopStep = None
        self._str_substep = None
        self._l_ARGS_KWS_DEFAULTS = []
        self._l_ARGS_KWS_BUILTINS = [
            {
                "kw": "reportShow",
                "default": False,
                "help": "show report at start of log",
                "argType": "bool",
            },
            {
                "kw": "reportTimes",
                "default": False,
                "help": "show step times in log",
                "argType": "bool",
            },
            {
                "kw": "reportEnv",
                "default": False,
                "help": "Override. Just get maya env info",
                "argType": "bool",
            },
            {
                "kw": "reportToDo",
                "default": False,
                "help": "Override. Get to do list for func",
                "argType": "bool",
            },
            {
                "kw": "printHelp",
                "default": False,
                "help": "Override.  Get help block for func",
                "argType": "bool",
            },
            {
                "kw": "setLogLevel",
                "default": None,
                "help": "Set the debug level on call",
                "argType": "str",
            },
            {
                "kw": "autoProgressBar",
                "default": False,
                "help": "Show generated progress bar by steps",
                "argType": "bool",
            },
            {
                "kw": "stopAtStep",
        ...
        self._l_reportMask = [
            "_b_ReturnBreak",
            "_b_ExceptionInterupt",
            "_b_pushCleanKWs",
            "_str_lastLog",
            "_l_ARGS_KWS_BUILTINS",
            "_l_toDo",
            "_l_warnings",
            "_l_errors",
            "_str_step",
    ...
    def __updateFuncStrings__(self):
        ...
        try:
            self._str_funcName = "{0}- from {1}".format(
                self._str_funcName, kws["calledFrom"]
            )
        except:
            pass
    ...
    def __dataBind__(self, *args, **kws):
        ...
        try:
            ...
            self._d_funcKWs = {}
        try:
            self._str_funcArgs = str(args)
        except:
            self._str_funcArgs = None
        try:
            self._str_funcKWs = str(kws)
        except:
            self._str_funcKWs = None

        try:
            self._str_funcHelp
        except:
            try:
                self._str_funcHelp = self.__doc__
            except:
        ...
        if self._l_ARGS_KWS_DEFAULTS:
            ...
            int_argIdx = 0
            for i, d_buffer in enumerate(
                self._l_ARGS_KWS_DEFAULTS + self._l_ARGS_KWS_BUILTINS
            ):
                str_kw = d_buffer["kw"]
                ...
                if str_kw not in self._d_funcKWs:
                    try:
                        ...
                        if d_buffer not in self._l_ARGS_KWS_BUILTINS:
                            try:
                                self.d_kws[d_buffer["kw"]] = args[
                                    int_argIdx
                                ]  # First we try the arg index
                            except:
                                # self.log_info("Using default [%s] = %s"%(str_kw,d_buffer.get("default")))
                                self.d_kws[str_kw] = d_buffer.get(
                                    "default"
                                )  # Lastly, we use the default value
                        else:
                            self.d_kws[str_kw] = d_buffer.get(
                                "default"
                            )  # Lastly, we use the default value
                    ...
                    int_argIdx -= (
                        1  # We'll be keep pushing down the arg start as we find kws
                    )
        ...
        for kw in kws:
            try:
                if kw not in l_storedKeys:
                    self.d_kws[kw] = kws[kw]
            except Exception as error:
                raise Exception(
                    "{0} failed to store kw: {1} | value: {2} | error: {3}".format(
                        self._str_reportStart, kw, kws[kw], error
                    )
                )
        """
    ...
    def validate_stopAtStep(self):
        ...
        if __stopAtStep is not None:
            if type(__stopAtStep) is int:
                ...
                if __stopAtStep == 0:
                    self.log_warning(
                        "a stopAtStep of 0 is pointless as no steps will execute. Will stop anyway."
                    )
                if __stopAtStep <= (len(self.l_funcSteps) - 1):
                    self.log_debug("stopAtStep in range")
                    self._int_stopStep = __stopAtStep
                else:
                    self.log_debug("stopAtStep NOT in range")
    ...
    def _ExceptionHook_(self, etype=None, value=None, tb=None, detail=None):
        ...
        try:
            ...
            if detail == 2:
                ...
                self.report_base()
                self.log_info(
                    _str_headerDiv + " Exception Log " + _str_headerDiv + _str_subLine
                )
                ...
                if str_lastLogBuffer:
                    try:
                        self.log_info("Last log entry: %s" % str_lastLogBuffer)
                    except Exception as error:
                        log.error("This failed")
                        log.error("Failed to report last log: {0}".format(error))
                for i, item in enumerate(
                    reversed(inspect.getouterframes(tb.tb_frame)[1:])
                ):
            ...
            print(
                (
                    "[{0}._ExceptionHook_ Exception | {1}".format(
                        self._str_funcCombined, error
                    )
                )
            )
    ...
    def go(self, *args, **kws):
        ...
        t_start = time.time()
        try:
            if not self.l_funcSteps:
                self.l_funcSteps = [{"call": self.__func__}]
            int_keys = list(range(0, len(self.l_funcSteps) - 1))
            self.int_max = len(self.l_funcSteps) - 1
        except Exception as error:
            raise Exception(
                ">" * 3
                + " {0}[FAILURE go start | error: {1}]".format(
                    self._str_funcCombined, error
                )
            )
        ...
        for i, d_step in enumerate(self.l_funcSteps):
            ...
            if self._int_stopStep is not None and i >= self._int_stopStep:
                self.log_info(
                    "Stop at step reached ({0}) | Skipped the following...".format(
                        self._int_stopStep
                    )
                )
                for ii, d_step in enumerate(self.l_funcSteps[self._int_stopStep :]):
                    ...
                    self.log_info(
                        "Step: {0} | {1}".format(ii + self._int_stopStep, _str_step)
                    )
                break
            t1 = time.time()
            try:
                try:
                    _str_step = d_step.get("step", False)
                    if _str_step:
                        self._str_progressBarReportStart = (
                            self._str_reportStart
                        )  # + " %s "%_str_step
                    else:
                        _str_step = "Process"
                    ...
                    raise Exception("[strStep query]{%s}" % error)

                try:
                    if self._b_autoProgressBar:
                        self.progressBar_set(
                            status=_str_step, progress=i, maxValue=int_lenSteps
                        )
                except Exception as error:
                    self.log_warning("[progress bar! | error: {0}]".format(error))
		...
		if goTo.lower() == str_name:
            ...
            if self._b_reportTimes:
                self.log_info(
                    " [TIME] -- Step: '{0}' >>  {1} ".format(pair_time[0], pair_time[1])
                )
        self.progressBar_end()
        ...
        if self._b_reportTimes:
            try:
                ...
                if int_lenSteps > 2:
                    self.log_info(
                        _str_headerDiv + " Times " + _str_headerDiv + _str_subLine
                    )
                    # if self.int_max != 0:
                    ...
                    self.log_warning(
                        _str_headerDiv
                        + " Total : %0.3f sec " % (f_total)
                        + _str_headerDiv
                        + _str_subLine
                    )
                # else:self.log_warning("[Total = %0.3f sec] " % (f_total))
            except Exception as error:
                self.log_error("[Failed to report times | error: {0}]".format(error))
    ...
    def _return_(self):
        ...
        if self.int_max == 0:  # If it's a one step, return, return the single return
            try:
                return self.d_return[list(self.d_return.keys())[0]]
            except:
                pass

        for k in list(
            self.d_return.keys()
        ):  # Otherise we return the first one with actual data
    ...
    def subTimer(self, func, *args, **kws):
        ...
        if self._b_reportTimes:
            self.log_info(
                " [TIME] -- subStep: '{0}' >>  {1} ".format(_str_substep, _str_time)
            )
        return res
    ...
    def report_base(self):
        ...
        try:
            self.log_info(
                _str_baseStart + " Python Module Version: %s " % self.mod.__version__
            )
        except:
            pass
        self.log_info(_str_headerDiv + " ArgsKws " + _str_headerDiv + _str_subLine)
        if self._str_funcArgs:
            self.log_info(" Args: %s" % self._str_funcArgs)
        if self._str_funcKWs:
            self.log_info(" KWs: %s" % self._str_funcKWs)
        """if self.d_kws:
    ...
    def report_selfStored(self):
        ...
        if l_keys:
            self.log_info(
                _str_headerDiv + " Self Stored " + _str_headerDiv + _str_subLine
            )
            for k in l_keys:
                if k not in self._l_reportMask:
                    ...
                    if type(buffer) is dict:
                        ...
                        for k2 in l_bufferKeys:
                            self.log_info(
                                _str_baseStart * 2 + "[%s] = %s " % (k2, buffer[k2])
                            )
                    else:
    ...
    def report_steps(self):
        if self.l_funcSteps:
            ...
            for i, d in enumerate(self.l_funcSteps):
                try:
                    self.log_info("'%s' : %s " % (i, d.get("step")))
                except:
                    pass
    ...
    def report_warnings(self):
        if self._l_warnings:
            self.log_info(
                _str_headerDiv + " Warnings : " + _str_headerDiv + _str_subLine
            )
    ...
    def report_argsKwsDefaults(self):
        if self._l_ARGS_KWS_DEFAULTS:
            ...
            for i, d_buffer in enumerate(self._l_ARGS_KWS_DEFAULTS):
                ...
                try:
                    l_tmp.append(["kw", "'%s'" % d_buffer.get("kw")])
                except:
                    pass
                try:
                    l_tmp.append(["default", d_buffer.get("default")])
                except:
                    pass
                try:
                    l_tmp.append(["argType", d_buffer.get("argType")])
                except:
                    pass
    ...
    def printHelp(self):
        ...
        for i, d_buffer in enumerate(
            ...
            try:
                l_tmp.append("'%s'" % d_buffer["kw"])
            except:
                pass
            # arg default/type -------------------------------
            l_buffer = ["("]
            try:
                l_buffer.append("%s - " % (d_buffer["argType"]))
            except:
                pass
            try:
                l_buffer.append("%s" % (d_buffer["default"]))
            except:
                pass
            l_buffer.append(")")
    ...
    def set_logging(self, arg):
        try:
            d_logging = {"info": logging.INFO, "debug": logging.DEBUG}
    ...
    def log_toDo(self, arg):
        try:
            try:
                self._l_toDo.append("%s | %s" % (self._str_step, str(arg)))
            except:
                self._l_toDo.append("%s" % (str(arg)))
    ...
    def log_error(self, arg):
        try:
            try:
                self._l_errors.append("%s | %s" % (self._str_step, str(arg)))
            except:
                self._l_errors.append("%s" % (str(arg)))
    ...
    def log_warning(self, arg):
        try:
            try:
                self._l_warnings.append("%s | %s" % (self._str_step, str(arg)))
            except:
                self._l_warnings.append("%s" % (str(arg)))
    ...
    def log_debug(self, arg):
        try:
            if self._str_step:
                log.debug(
                    "[DEBUG]{0}{1} | {2}".format(
                        self._str_reportStart, self._str_step, str(arg)
                    )
                )
            else:
    ...
    def progressBar_start(
        ...
        self._str_progressBar = doStartMayaProgressBar(
            stepMaxValue, statusMessage, interruptableState
        )

    def progressBar_iter(self, **kws):
        if not self._str_progressBar:
            self.progressBar_start()
        if kws.get("status"):
            str_bfr = kws.get("status")
            kws["status"] = "%s > %s" % (self._str_progressBarReportStart, str_bfr)
        if "step" not in list(kws.keys()):
            kws["step"] = 1
        if "beginProgress" not in list(kws.keys()):
            kws["beginProgress"] = 1
        kws["edit"] = 1
    ...
    def progressBar_end(self, **kws):
        try:
            doEndMayaProgressBar(self._str_progressBar)
        except:
            pass
    ...
    def progressBar_setMaxStepValue(self, int_value):
        ...
        try:
            mc.progressBar(
                self._str_progressBar, edit=True, progress=0, maxValue=int_value
            )
        except Exception as error:
            log.error(
                "%s > failed to set progress bar maxValue | %s"
                % (self._str_reportStart, error)
            )
    ...
    def progressBar_setMinStepValue(self, int_value):
        if not self._str_progressBar:
            self.progressBar_start()
        try:
            mc.progressBar(self._str_progressBar, edit=True, minValue=int_value)
        except Exception as error:
            log.error(
                "%s > failed to set progress bar minValue | %s"
                % (self._str_reportStart, error)
            )
    ...
    def progressBar_set(self, **kws):
        ...
        try:
            ...
            log.error(
                "%s > failed to set progress bar status | %s"
                % (self._str_reportStart, error)
            )
    ...
    def log_infoDict(self, arg=None, tag="Stored Dict"):
        ...
        try:
            ...
            try:
                ...
                for k in l_keys:
                    try:
                        str_key = k.p_nameShort
                    except:
                        str_key = k
                    buffer = arg[k]
                    if isinstance(buffer, dict):
                        self.log_info(
                            " Nested Dict: '{0}' ".format(str_key) + _str_subLine
                        )
                        l_bufferKeys = list(buffer.keys())
                        l_bufferKeys.sort()
                        for k2 in l_bufferKeys:
                            self.log_info(
                                "-" * 3 + ">" + " '{0}' : {1} ".format(k2, buffer[k2])
                            )
                    else:
                        self.log_info(">" + " '{0}' : {1} ".format(str_key, arg[k]))
            except Exception as error:
                self.log_warning(
                    "[Not a dict. arg: {0} | error: {1} ]".format(arg, error)
                )
        except:
            pass
            ...
            for atr in arg:
                try:
                    ...
                    for k in l_keys:
                        ...
                        if type(buffer) is dict:
                            self.log_info(
                                ">" + " Nested Dict: '%s' " % (str_key) + _str_subLine
                            )
                            l_bufferKeys = list(buffer.keys())
                            l_bufferKeys.sort()
                            for k2 in l_bufferKeys:
                            ...
                            self.log_info(
                                ">" + " '%s' : %s " % (str_key, self.__dict__[atr][k])
                            )
                except Exception as error:
...
def returnCallerFunctionName():
    ...
    try:
        frame, filename, line_no, s_funcName, lines, index = inspect.getouterframes(
    ...
    class fncWrap(cgmFuncCls):
        ...
        def __func__(self):
            ...
            try:
                ...
                raise Exception("[ str_side: %s]{%s}" % (arg, error))
...
def get_mayaEnviornmentDict():
    ...
    for kw in [
        ...
        try:
            _d[kw] = mel.eval("about -%s" % kw)
        except Exception as error:
            log.error("%s | %s" % (kw, error))
    return _d
...
def report_enviornment():
    ...
    for kw, item in _d.items():
        try:
            print((_str_baseStart + " Maya %s : %s " % (kw, item)))
        except Exception as error:
            log.error("%s | %s" % (kw, error))
    ...
    def __call__(self, *args):
        try:
            ...
            try:
                ...
                log.info("Func: {0}".format(self._func))
            if self._args:
...
def stringModuleClassCall(self, module=None, func="", *args, **kws):
    ...
    try:
        ...
        _short = self.__class__.__name__
    ...
    try:
        ...
        log.debug(
            "|{0}| >> {1}.{2}({3}{4})...".format(
                _str_func, _short, func, _str_args, _kwString
            )
        )
...
def funcClassWrap(funcClass):
    ...
    def wrapper(*args, **kws):
        ...
        try:
            _str_funcName = args[0]._str_funcName
        except Exception as error:
            log.info(error)

        t1 = time.time()
        try:
            res = funcClass(*args, **kws)
        except Exception as error:
            ...
            log.info(
                "%s >> Time to Fail >> = %0.3f seconds "
                % (_str_funcName, (time.time() - t1))
                + _str_subLine
            )
            raise error
        t2 = time.time()
        ...
        log.info(
            "%s >> Time >> = %0.3f seconds " % (_str_funcName, (t2 - t1)) + _str_subLine
        )
        return res
...
def func_getTraceString(func):
    ...
    try:
        ...
        if mClass:
            _l_join.append(mClass)
        if mNode:
            _l_join.append("('{0}')".format(mNode))
...
def Timer2(func):
    ...
    def wrapper(*args, **kws):
        ...
        functionTrace += func.__name__
        if str_arg:
            functionTrace += "(%s)" % str_arg
        log.info(
            ">" * 3 + " TIMER : %s: %0.4f sec " % (functionTrace, (t2 - t1)) + "<" * 3
        )
...
def returnTimeStr(arg="%m%d%Y"):
    ...
    try:
        ...
        raise Exception(
            "cgmGeneral.returnDateStr(arg = {1}) fail | {0}".format(error, arg)
        )


def doStartMayaProgressBar(
    stepMaxValue=100, statusMessage="Calculating....", interruptableState=True
):
    """
    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    DESCRIPTION:
...
def Wrap_exception(func):
    ...
    def Exception_Wrapper(*args, **kws):
        res = None
        try:
            _str_func = func_getTraceString(func)
        except:
            _str_func = func

        log.debug(logString_start(_str_func))
...
def func_snapShot(dat=None):
    ...
    if dat is None:
        print(
            (
                "[ {0} ]snapShot || No dat provided".format(
    ...
    print(
        (
            _str_headerDiv
            + " call: {0}   ".format(sys._getframe(1).f_code.co_name)
            + _str_headerDiv
            + _str_subLine
        )
    )
