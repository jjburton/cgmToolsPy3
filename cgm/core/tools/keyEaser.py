...
class ui(cgmUI.cgmGUI):
    ...
    def buildColumn_noise(self,parent, asScroll = False):
        ...
        self.uiTF_noise_amplitude(edit=True, changeCommand=cgmGEN.Callback(self.uiFunc_setAmplitudeSlider, "field"))
        
        self.uiSlider_noise_amplitude = mUI.MelFloatSlider(_row,0.0, 10.0, defaultAmplitude, step=0.01)
        self.uiSlider_noise_amplitude.setValue(defaultAmplitude)
        cmd = cgmGEN.Callback(self.uiFunc_setAmplitudeSlider, "slider")

        self.uiSlider_noise_amplitude(edit=True, changeCommand=cgmGEN.Callback(self.uiFunc_handleChange, cmd))
        self.uiSlider_noise_amplitude(edit=True, dragCommand=cgmGEN.Callback(self.uiFunc_handleDrag, cmd))
        ...
        self.uiTF_noise_frequency(edit=True, changeCommand=cgmGEN.Callback(self.uiFunc_setFrequencySlider, "field"))
        
        self.uiSlider_noise_frequency = mUI.MelFloatSlider(_row,0.0, 1.0, defaultFrequency, step=0.01)
        self.uiSlider_noise_frequency.setValue(defaultFrequency)
        cmd = cgmGEN.Callback(self.uiFunc_setFrequencySlider, "slider")
        
        self.uiSlider_noise_frequency(edit=True, changeCommand=cgmGEN.Callback(self.uiFunc_handleChange, cmd))
        self.uiSlider_noise_frequency(edit=True, dragCommand=cgmGEN.Callback(self.uiFunc_handleDrag, cmd))
    ...
    def uiFunc_handleDrag(self, func):
        if not self._isChunkOpen:
            ...
            self.scriptJobs = []
            self.scriptJobs.append(mc.scriptJob( e= ["Undo",cmd], protected=True, ro=True))
            self.scriptJobs.append(mc.scriptJob( e= ["SelectionChanged",cmd], protected=True, ro=True))
    ...
    def uiFunc_setFavorSlider(self, source):
        ...
        adjust_favoring(self._selected_objects, self.uiSlider_favor.getValue(), self.uiSlider_midPoint.getValue())
    ...
    def uiFunc_setAmplitudeSlider(self, source):
        uiFunc_setFieldSlider(self.uiTF_noise_amplitude, self.uiSlider_noise_amplitude, source, 10.0, .01)
    ...
    for object_dict in selected_objects:
        ...
        for obj, attr_dict in object_dict.items():
            for attr, keys in attr_dict.items():
                ...
                for key in keys:
                    new_value = key[1] + noise.noise(key[0] * frequency + keys[int(len(keys)/2)][0]/30.0, keys[0][1]) * amplitude
                    # Set the new key value
                    attr_full_name = "{}.{}".format(obj, attr)
                    mc.setKeyframe(attr_full_name, time=key[0], value=new_value)
