...
def validate_attrTypeMatch(t1,t2):
    ...
    for o in list(_d_attrTypes.keys()):
...
def set(node, attr = None, value = None, lock = False,force=True, **kws):
    ...
    if _aType in ['TdataCompound']:
        ...
        if '[' in _attr:
            ...
            _children = get_children(_d)
            for idx,lv in list(value.items()):
...
def validate_attrTypeName(attrType):
    ...
    for option in list(_d_attrTypes.keys()):
...
def get_compatible(node,attr,targetNode, direction = 'to'):
    ...
    for a in mc.listAttr(targetNode):
        try:
            ...
            if direction == 'to':
                for k,types in list(_d_compatibility.items()):
                    log.debug(k + ' ' + _type)
                    if _type in types:
                        log.debug(k)
...
def get_compatibilityDict(report = False):
    ...
    for o in _l_created:
        ...
        for k in list(_d.keys()):
            if k not in list(_d_attrsByType.keys()):
                _d_attrsByType[k] = []
            for a in _d[k]:
    ...
    for k in list(_d_attrsByType.keys()):
        _d_compatible[k] = {'out':[],'in':[]}
        for k2 in list(_d_attrsByType.keys()):
            for n in list(_d_nodePairs.keys()):
