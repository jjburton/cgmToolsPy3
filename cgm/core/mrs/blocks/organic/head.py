...
def create_jointHelpers(self,cnt=None, force = False):
    ...
    if not force:
        if cnt and len(ml_jointHelpers) == cnt:
            return True
        if not cnt:
            if cnt_target == len(ml_jointHelpers):
                return True
    ...
    if cnt:
        self.neckJoints = MATH.Clamp(cnt - 1,1)#neck - head
    ...
    _l_baseNames = ATTR.datList_get(self.mNode, 'nameList')
    _l_names = self.atUtils('skeleton_getNameDicts',False, self.neckJoints +1,
                                 iterName = _l_baseNames[0], cgmType='jointHelper')      
    
    l_pcts = [i*(1.0/(self.neckJoints)) for i in range(self.neckJoints)] + [1.0]
    
    pprint.pprint([l_pcts,_l_names,self.neckJoints])
    ...
    def attachToCurve2(mHandle,mCrv):
        if not mHandle.getMessage('trackGroup'):
            mHandle.doGroup(True,True,asMeta=True,typeModifier = 'track',setClass='cgmObject')
...
def skeleton_build(self, forceNew = True):
    try:
        ...
        if self.neckBuild:
            p = POS.get( ml_jointHelpers[-1].mNode )        
        else:
        ...
        if self.neckBuild:
            ...
            mHead_jnt.doSnapTo(self.jointHelper.mNode)
            """
        ...
        if self.neckBuild:#...Neck =====================================================================
            ...
            ml_joints = JOINT.build_chain([mObj.p_position for mObj in ml_jointHelpers][:-1], parent=True, worldUpAxis= mVec_up)
...
def rig_prechecks(self):
    try:
        ...
        if mBlock.ikMidSetup and mBlock.neckJoints < 2:
            mBlock.ikMidSetup = False
            self.l_precheckWarnings.append("Must have more than one neck joint with ikMidSetup, turning off")    
...
def rig_segments(self):
    try:
        ...
        if self.ml_ikMidControls and mBlock.neckControls == 1:
            ...
            ml_influences = [ml_influences[0]] + self.ml_ikMidControls + [ml_influences[-1]]
        ...
        if self.ml_ikMidControls:
            _d['sectionSpans'] = 1 + len(self.ml_ikMidControls)
            ...
            if self.mBlock.neckIK:
                ...
                if mBlock.neckControls == 1:
                    ...
                    if self.ml_ikMidControls:
                        ...
                        ml_midTrackJoints = [ml_midTrackJoints[0]] + self.ml_ikMidControls + [ml_midTrackJoints[-1]]
                        ...
                        RIGFRAME.segment_mid(self,self.ml_ikMidControls,ml_handleJoints,mRoot,
                ...
                if self.b_scaleSetup:
                    ...
                    for mJnt in ml_ikJoints[1:]:
                        if _ikNeck not in ['spline','curve']:mJnt.p_parent = mIKGroup
                        mJnt.segmentScaleCompensate = False
...
def build_proxyMesh(self, forceNew = True, puppetMeshMode = False, skin = False):
    ...
    #>> Head ===================================================================================
    log.debug("|{0}| >> Head...".format(_str_func))       
    if directProxy and not puppetMeshMode:
        for mJnt in ml_rigJoints:
