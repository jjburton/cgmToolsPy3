...
class ui(cgmUI.cgmGUI):
    ...
    def insert_init(self, *args, **kws):
        ...
        self.var_lastProject = cgmMeta.cgmOptionVar(
            "cgmVar_projectCurrent", varType="string"
        )
        self.var_lastAsset = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_last_asset", varType="string"
        )
        self.var_lastSubtype = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_last_subtype", varType="string"
        )
        self.var_lastSet = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_last_set", varType="string"
        )
        self.var_lastVariation = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_last_variation", varType="string"
        )
        self.var_lastVersion = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_last_version", varType="string"
        )
        self.var_showAllFiles = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_show_all_files", defaultValue=1
        )
        ...
        self.var_useMayaPy = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_use_mayaPy", defaultValue=1
        )
        self.var_categoryStore = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_category", defaultValue=0
        )
        self.var_subTypeStore = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_subType", defaultValue=0
        )
        self.var_alwaysSendReferenceFiles = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_alwaysSendReferences", varType="int", defaultValue=0
        )
        self.var_showDirectories = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_show_directories", defaultValue=0
        )
        self.var_displayDetails = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_display_details", defaultValue=1
        )
        self.var_displayProject = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_display_project", defaultValue=1
        )
        ...
        self.var_posePathLocal = cgmMeta.cgmOptionVar(
            "cgmVar_mrs_localPosePath", defaultValue=""
        )
        self.var_posePathProject = cgmMeta.cgmOptionVar(
            "cgmVar_mrs_projectPosePath", defaultValue=""
        )
        self.var_updateRigs = cgmMeta.cgmOptionVar(
            "cgmVar_sceneUI_updateRigs", defaultValue=0
        )

        self.var_bakeSet = cgmMeta.cgmOptionVar(
            "cgm_bake_set", varType="string", defaultValue="bake_tdSet"
        )
        self.var_deleteSet = cgmMeta.cgmOptionVar(
            "cgm_delete_set", varType="string", defaultValue="delete_tdSet"
        )
        self.var_exportSet = cgmMeta.cgmOptionVar(
            "cgm_export_set", varType="string", defaultValue="export_tdSet"
        )
        ...
        self.var_project = cgmMeta.cgmOptionVar(
            "cgmVar_projectCurrent", defaultValue=""
        )
        self.var_pathProject = cgmMeta.cgmOptionVar(
            "cgmVar_projectPath", defaultValue=""
        )
        self.var_pathLastProject = cgmMeta.cgmOptionVar(
            "cgmVar_projectLastPath", defaultValue=""
        )
    ...
    def rebuild_scriptUI(self):
        ...
        if not os.path.exists(_path):
            return log.warning(
                cgmGEN.logString_msg(_str_func, "path doesn't exist: {}".format(_path))
            )
            ...
            if self.hasSub:
                if self.hasVariant:
                    return (
                        os.path.normpath(
                            os.path.join(
                                self.path_variationDirectory,
                                self.versionList["scrollList"].getSelectedItem(),
                            )
                        )
    ...
    def exportFileName(self):
        if self.hasSub:
            if self.hasVariant:
                return "{0}_{1}_{2}.fbx".format(
                    self.assetList["scrollList"].getSelectedItem(),
                    self.subTypeSearchList["scrollList"].getSelectedItem(),
                    self.variationList["scrollList"].getSelectedItem(),
                )
            else:
                return "{0}_{1}.fbx".format(
                    self.assetList["scrollList"].getSelectedItem(),
                    self.subTypeSearchList["scrollList"].getSelectedItem(),
                )
        else:
            return "{0}_{1}.fbx".format(
                self.assetList["scrollList"].getSelectedItem(), self.subType
            )
    ...
    def category(self):
        ...
        log.debug(log_start(_str_func))
        return (
            self.categoryList[self.categoryIndex]
            if len(self.categoryList) > self.categoryIndex
            else self.categoryList[0]
        )
    ...
    def subType(self):
        ...
        return (
            self.subTypes[min(self.subTypeIndex, len(self.subTypes) - 1)]
            if self.subTypes
            else None
        )
    ...
    def LoadOptions(self, *args):
        ...
        self.setTitle(
            "|[ {} ]| --- {}".format(
                self.mDat.d_project.get("name", "No Name"), self.WINDOW_TITLE
            )
        )
    ...
    def SaveOptions(self, *args):
        ...
        self.showAllFiles = (
            self.cb_showAllFiles(q=True, checkBox=True)
            if self.cb_showAllFiles
            else False
        )
        ...
        self.useMayaPy = (
            self.cb_useMayaPy(q=True, checkBox=True) if self.cb_useMayaPy else False
        )
        self.showDirectories = (
            self.cb_showDirectories(q=True, checkBox=True)
            if self.cb_showDirectories
            else False
        )
                ...
                if k in _dat["split"]:
                    ...
                    if numItemsFound > 0:
                        if l_temp[0] in self.categoryList:
                            ...
                            log.warning(
                                "{0} not found in category list".format(l_temp[0])
                            )
                            return
                    ...
                    if numItemsFound > 2:
                        if l_temp[2] in self.subTypes:
                            ...
                            log.warning(
                                "{0} not found in subType list".format(l_temp[2])
                            )
                            return
    ...
    def build_layoutWrapper(self, parent):
        ...
        self._detailsColumn = mUI.MelScrollLayout(
            _ParentForm, useTemplate="cgmUISubTemplate", w=294
        )
        self._projectForm = mUI.MelTabLayout(
            _ParentForm, w=400, ut="cgmUITemplate"
        )  # w180 mUI.MelFormLayout(_ParentForm,useTemplate = 'cgmUISubTemplate', w=250)
        ...
        self._detailsToggleBtn = mUI.MelButton(
            _MainForm,
            ut="cgmUITemplate",
            label="<",
            w=15,
            bgc=(1.0, 0.445, 0.08),
            c=lambda *a: mc.evalDeferred(self.uiFunc_toggleDisplayInfo, lp=True),
        )

        self._projectToggleBtn = mUI.MelButton(
            _MainForm,
            ut="cgmUITemplate",
            label=">",
            w=15,
            bgc=(1.0, 0.445, 0.08),
            c=lambda *a: mc.evalDeferred(self.uiFunc_toggleProjectColumn, lp=True),
        )

        _directoryColumn = mUI.MelColumnLayout(
            _MainForm, useTemplate="cgmUISubTemplate"
        )
        ...
        uiTab_Project = mUI.MelScrollLayout(
            ui_tabs, ut="cgmUITemplate"
        )  # mUI.MelColumnLayout(ui_tabs)
        uiTab_Content = mUI.MelFormLayout(
            ui_tabs, ut="cgmUITemplate"
        )  # mUI.MelColumnLayout(ui_tabs)
        self.uiTab_Content = uiTab_Content
        uiTab_Export = mUI.MelFormLayout(
            ui_tabs, ut="cgmUITemplate"
        )  # mUI.MelScrollLayout( ui_tabs,ut='cgmUITemplate' )
        ...
        self.ui_projectDirty = mUI.MelButton(
            uiTab_Project,
            label="Changes detected. Save?",
            vis=False,
            height=15,
            bgc=PROJECT._colorBad,
            command=cgmGEN.Callback(self.uiProject_saveAndRefresh),
        )

        PROJECT.buildFrame_baseDat(
            self,
            uiTab_Project,
            changeCommand=cgmGEN.Callback(self.uiFunc_projectDirtyState, True),
        )

        PROJECT.buildFrame_assetTypes(
            self,
            uiTab_Project,
            changeCommand=cgmGEN.Callback(self.uiFunc_projectDirtyState, True),
        )

        PROJECT.buildFrame_paths(
            self,
            uiTab_Project,
            changeCommand=cgmGEN.Callback(self.uiFunc_projectDirtyState, True),
        )
        ...
        # Scroll list
        mScrollList = Project.cgmProjectDirList(
            uiTab_Content,
            ut="cgmUISubTemplate",
        ...
        try:
            mScrollList(edit=True, hlc=[0.5, 0.5, 0.5])
        except:
            pass

        mScrollList.set_filterObj(_textField)
        _textField(edit=True, tcc=lambda *a: mScrollList.update_display())
        ...
        # Scroll list
        mScrollList2 = Project.cgmProjectDirList(
            uiTab_Export,
            ut="cgmUISubTemplate",
            allowMultiSelection=0,
            en=True,
        ...
        try:
            mScrollList2(edit=True, hlc=[0.5, 0.5, 0.5])
        except:
            pass

        mScrollList2.set_filterObj(_textField)
        _textField(edit=True, tcc=lambda *a: mScrollList2.update_display())
        ...
        # Main Asset Lists
        ##############################
        self._assetsForm = mUI.MelFormLayout(
            _MainForm, ut="cgmUISubTemplate", numberOfDivisions=100
        )  # mc.columnLayout(adjustableColumn=True)

        # Category
        _catForm = mUI.MelFormLayout(self._assetsForm, ut="cgmUISubTemplate")
        self.categoryBtn = mUI.MelButton(
            _catForm,
            label=self.category,
            ut="cgmUITemplate",
        ...
        self.assetList = self.build_searchable_list(
            _catForm, sc=self.uiFunc_assetList_select
        )
        self.assetTSLpum = mUI.MelPopupMenu(
            self.assetList["scrollList"], pmc=self.UpdateAssetTSLPopup
        )

        mRow_asset = mUI.MelHLayout(_catForm, padding=2)
        self.assetButton = mUI.MelButton(
            mRow_asset, ut="cgmUITemplate", label="New Asset", command=self.CreateAsset
        )
        ...
        self.subTypeBtn = mUI.MelButton(
            _setsForm,
            label=self.subType,
            ut="cgmUITemplate",
            ann="Select the sub type",
            en=True,
        )

        self.subTypeMenu = mUI.MelPopupMenu(self.subTypeBtn, button=1)
        self.subTypeSearchList = self.build_searchable_list(
            _setsForm,
            sc=self.uiFunc_subTypeList_select,
            refreshCommand=lambda *a: self.LoadSubTypeList(),
        )

        # JOSH HERE
        pum = mUI.MelPopupMenu(
            self.subTypeSearchList["scrollList"],
        ...
        self.ml_dirOptions_set.append(
            mUI.MelMenuItem(
                pum, label="Rename Set", command=partial(self.rename_below, "set")
            )
        )

        self.ml_fileOptions_set = []

        self.ml_fileOptions_set.append(
            mUI.MelMenuItem(
                pum,
                label="Reference",
        ...
        self.uiPop_sendToProject_sub = mUI.MelMenuItem(
            pum, label="Send To Project", subMenu=True, en=1
        )
        ...
        self.ml_fileOptions_set.append(
            mUI.MelMenuItem(
                pum, label="Send Last To Queue", command=self.AddLastToExportQueue
            )
        )
        self.ml_fileOptions_set.append(
            mUI.MelMenuItem(
                pum,
                label="Create SubTypeRef",
                command=lambda *a: self.CreateSubTypeRef(),
            )
        )
        ...
        for t in ["export", "rig", "cutscene"]:
            mUI.MelMenuItem(
                _batch, label=t.capitalize(), command=partial(self.AddToExportQueue, t)
            )
        ...
        mUI.MelMenuItem(
            pum,
            label="Delete",
            command=lambda *a: self.uiFunc_deleteSelectedInList("sets"),
        )
        ...
        self.subTypeButton = mUI.MelButton(
            mRow_sets,
            ut="cgmUITemplate",
            label="Save Version",
            command=self.SaveVersion,
        )
        ...
        _variationBtn = mUI.MelButton(
            _variationForm,
            label="Variation",
            ut="cgmUITemplate",
            ann="Select the asset variation",
            en=False,
        )

        self.variationList = self.build_searchable_list(
            _variationForm,
            sc=self.uiFunc_variationList_select,
            refreshCommand=lambda *a: self.LoadVariationList(),
        )

        pum = mUI.MelPopupMenu(
            self.variationList["scrollList"],
        ...
        self.ml_dirOptions_variant.append(
            mUI.MelMenuItem(
                pum,
                label="Rename Variant",
                command=partial(self.rename_below, "variant"),
            )
        )

        self.ml_fileOptions_variant = []

        self.ml_fileOptions_variant.append(
            mUI.MelMenuItem(
                pum,
        ...
        self.ml_fileOptions_variant.append(
            mUI.MelMenuItem(
                pum, label="Import", ann=_d_ann.get("import"), command=self.ImportFile
            )
        )
        self.ml_fileOptions_variant.append(
            mUI.MelMenuItem(
        ...
        self.uiPop_sendToProject_variant = mUI.MelMenuItem(
            pum, label="Send To Project", subMenu=True
        )
        ...
        mUI.MelMenuItem(
            pum, label="Refresh", command=lambda *a: self.LoadVariationList()
        )
        ...
        mUI.MelMenuItem(
            pum,
            label="Delete",
            command=lambda *a: self.uiFunc_deleteSelectedInList("variation"),
        )
        # ---------------------------------------------------------------------------------------

        self.variationButton = mUI.MelButton(
            _variationForm,
            ut="cgmUITemplate",
            label="New Variation",
            command=self.CreateVariation,
        ...
        _versionBtn = mUI.MelButton(
            _versionForm,
            label="Version",
            ut="cgmUITemplate",
            ann="Select the asset version",
            en=False,
        )

        self.versionList = self.build_searchable_list(
            _versionForm,
            sc=self.uiFunc_versionList_select,
            refreshCommand=lambda *a: self.LoadVersionList(),
        )

        pum = mUI.MelPopupMenu(
            self.versionList["scrollList"],
        ...
        self.uiPop_sendToProject_version = mUI.MelMenuItem(
            pum, label="Send To Project", subMenu=True
        )
        mUI.MelMenuItem(pum, label="Send To Build", command=self.SendToBuild, en=1)
        ...
        mUI.MelMenuItem(
            pum, label="Create SubTypeRef", command=lambda *a: self.CreateSubTypeRef()
        )
        ...
        mUI.MelMenuItem(
            pum,
            label="Delete",
            command=lambda *a: self.uiFunc_deleteSelectedInList("version"),
        )

        self.versionButton = mUI.MelButton(
            _versionForm,
            ut="cgmUITemplate",
        ...
        mUI.MelButton(
            ...
            ut="cgmUITemplate",
            label="Bake",
            c=partial(self.RunExportCommand, 0),
            h=self.__itemHeight,
        )
        mUI.MelButton(
            _row,
            ut="cgmUITemplate",
            label="Rig",
            c=partial(self.RunExportCommand, 3),
            h=self.__itemHeight,
        )
        mUI.MelButton(
            _row,
            ut="cgmUITemplate",
            label="Cutscene",
            c=partial(self.RunExportCommand, 2),
            h=self.__itemHeight,
        )
        ...
        _row = mUI.MelHSingleStretchLayout(
            _bottomColumn, useTemplate="cgmUISubTemplate"
        )
        ...
        self.exportQueueFrame = mUI.MelFrameLayout(
            _bottomColumn, label="Export Queue", collapsable=True, collapse=True
        )
        ...
        _col = mUI.MelColumnLayout(
            _rcl, width=200, adjustableColumn=True, useTemplate="cgmUISubTemplate"
        ...
        mUI.MelButton(
            ...
            label="Save",
            ut="cgmUITemplate",
            command=partial(self.ExportQueue_write),
        )
        mUI.MelButton(
            _row,
            label="Load",
            ut="cgmUITemplate",
            command=partial(self.ExportQueue_load),
        )
        _row.layout()
        ...
        mUI.MelButton(
            ...
            label="Remove",
            ut="cgmUITemplate",
            command=partial(self.RemoveFromQueue, 0),
        )
        cgmUI.add_LineSubBreak()
        mUI.MelButton(
            _col,
            label="Remove All",
            ut="cgmUITemplate",
            command=partial(self.RemoveFromQueue, 1),
        )
        cgmUI.add_LineSubBreak()
        mUI.MelButton(
            _col,
            label="Batch Export",
            ut="cgmUITemplate",
            command=partial(self.batch_buildFile),
        )
        cgmUI.add_LineSubBreak()
        ...
        self.updateRigsCB = mUI.MelCheckBox(
            _c2,
            label="Update and Save",
            v=self.var_updateRigs.getValue(),
            cc=cgmGEN.Callback(self.var_updateRigs.toggle),
        )
        ...
        _rcl(
            ...
            attachControl=[(self.queueTSL, "right", 0, _col)],
        )
    ...
    def buildAssetForm(self):
        ...
        if not self.subTypes:
            ...
            mc.formLayout(
                self._subForms[2], e=True, vis=self.hasVariant and self.hasSub
            )
        ...
        for i, form in enumerate(attachedForms):
            ...
            if i == len(attachedForms) - 1:
                ...
                attachPosition.append(
                    (form, "right", 5, (100 / len(attachedForms)) * (i + 1))
                )
    ...
    def build_menus(self):
        ...
        self.uiMenu_FirstMenu = mUI.MelMenu(
            l="File", pmc=cgmGEN.Callback(self.buildMenu_first)
        )
        ...
        self.uiMenu_ToolsMenu = mUI.MelMenu(
            l="Tools", pmc=self.buildMenu_tools, pmo=True
        )
        self.uiMenu_Utils = mUI.MelMenu(
            l="Utils", pmo=1, pmc=cgmGEN.Callback(self.buildMenu_utils), tearOff=True
        )
        self.uiMenu_projectUtils = mUI.MelMenu(l="Project Scripts", tearOff=True)
    ...
    def uiProject_refreshDisplay(self):
        ...
        try:
            self.uiImage_ProjectRow(edit=True, bgc=_bgColor)
        except Exception as err:
        ...
        try:
            ...
            # print _c_secondary
            vTmp = _c_secondary
            vLite = [MATH.Clamp(1.7 * v, 0.5, 1.0) for v in vTmp]
            ...
            self._detailsToggleBtn(edit=True, bgc=(1.0, 0.445, 0.08))
            self._projectToggleBtn(edit=True, bgc=(1.0, 0.445, 0.08))
            self.uiScrollList_dirContent(edit=True, hlc=(1.0, 0.445, 0.08))
            self.uiScrollList_dirExport(edit=True, hlc=(1.0, 0.445, 0.08))
        ...
        if os.path.exists(d_userPaths["content"]):
            ...
            self.l_categoriesBase = (
                self.mDat.assetTypes_get()
                if self.mDat.assetTypes_get()
                else self.mDat.d_structure.get("assetTypes", [])
            )
    ...
    def buildMenu_first(self):
        ...
        for p in self.mPathList_recent.l_paths:
            ...
            mUI.MelMenuItem(_recent, l=_short, c=partial(self.LoadProject, p))
        # ==========================================================================================
        ...
        mUI.MelMenuItem(
            ...
            c=lambda *a: mc.evalDeferred(
                cgmGEN.Callback(PROJECT.uiProject_saveAs, self), lp=True
            ),
        )
        ...
        mUI.MelMenuItem(
            ...
            c=lambda *a: mc.evalDeferred(
                cgmGEN.Callback(self.uiProject_reset), lp=True
            ),
        )
    ...
    def buildMenu_project(self, *args):
        ...
        for i, p in enumerate(mPathList.mOptionVar.value):
            ...
            nameUse = (
                ...
                else "%s {%i}" % (name, project_names.count(name) - 1)
            )
    ...
    def buildMenu_options(self, *args):
        ...
        self.cb_useMayaPy = mUI.MelMenuItem(
            self.uiMenu_OptionsMenu,
            l="Use Maya Standalone",
        ...
        self.cb_showAllFiles = mUI.MelMenuItem(
            self.uiMenu_OptionsMenu,
            l="Show all files",
        ...
        self.cb_showDirectories = mUI.MelMenuItem(
            self.uiMenu_OptionsMenu,
            l="Show Directories",
            checkBox=self.showDirectories,
            c=lambda *a: mc.evalDeferred(self.SaveOptions, lp=True),
        )
        self.cb_alwaysSendReferenceFiles = mUI.MelMenuItem(
            self.uiMenu_OptionsMenu,
            l="Always Send References",
            checkBox=int(self.var_alwaysSendReferenceFiles.getValue()),
            c=lambda *a: mc.evalDeferred(
                self.var_alwaysSendReferenceFiles.toggle, lp=True
            ),
        )
    ...
    def uiFunc_assetList_select(self):
        ...
        if self.subTypes:
            ...
            if self.subType not in self.subTypes:
                log.debug(
                    log_msg(_str_func, "Setting subtype because stored not in list")
                )
    ...
    def uiUpdate_setsButtons(self):
        ...
        if self.hasSub:
            ...
            mUI.MelButton(
                self.mRow_setButtons,
                ut="cgmUITemplate",
                label="New {0}".format(self.subType.capitalize()),
                command=self.CreateSubAsset,
            )
    ...
    def buildProjectColumn(self):
        ...
        button_add = mUI.MelButton(
            _row, label="Add", ut="cgmUITemplate", ann="Add a subdir to the path root"
        )

        button_verify = mUI.MelButton(
            _row,
            label="Verify Dir",
            ut="cgmUITemplate",
        ...
        _textField = mUI.MelTextField(
            _inside, ann="Filter", w=50, bgc=[0.3, 0.3, 0.3], en=True, text=""
        )
        ...
        button_verify(
            edit=True,
            c=lambda *a: Project.uiProject_verifyDir(
                self, "content", None, mScrollList
            ),
        )
        button_add(
            edit=True,
    ...
    def buildDetailsColumn(self):
        ...
        mUI.MelMenuItem(
            pum, label="Remake Thumbnail", command=cgmGEN.Callback(self.makeThumbnail)
        )

        self.uiButton_MakeThumb = mUI.MelButton(
            self._detailsColumn,
            ut="cgmUITemplate",
            h=150,
            label="Make Thumbnail",
            c=cgmGEN.Callback(self.makeThumbnail),
            vis=(thumb == None),
        )
        ...
        _d = {
            "Asset": self.assetMetaData.get("asset", None),
        ...
        for k in ["Asset", "Type", "SubAsset", "Variation", "User"]:
            ...
            if _dat is not None:
                ...
                _row.setStretchWidget(
                    mUI.MelTextField(
                        _row, text=_dat, ann=_dat, editable=False, bgc=_bgc
                    )
                )
        ...
        _row = mUI.MelHSingleStretchLayout(self._detailsColumn)
        mUI.MelSpacer(_row, w=_spacer)
        noteField = mUI.MelScrollField(
            _row,
            h=150,
            text=self.assetMetaData.get("notes", ""),
            wordWrap=True,
            editable=True,
            bgc=(0.8, 0.8, 0.8),
        )
        ...
        if self.assetMetaData.get("references", None):
            ...
            for ref in self.assetMetaData.get("references", []):
                ...
                _row.setStretchWidget(
                    mUI.MelTextField(
                        _row, text=ref, editable=False, bgc=(0.8, 0.8, 0.8)
                    )
                )
                cgmUI.add_Button(
                    _row, "Load", cgmGEN.Callback(VALID.fileOpen, path, True, True)
                )
        ...
        if self.assetMetaData.get("shots", None):
            ...
            for shot in self.assetMetaData.get("shots", []):
                ...
                _ann = "{0} | start: {1} | end: {2} | length: {3}".format(
                    shot[0], shot[1][0], shot[1][1], shot[1][2]
                )
                mUI.MelSpacer(_row, w=_spacer)
                mUI.MelTextField(
                    _row,
    ...
    def metaData_copyShotList(self):
        ...
        _d.get("file")
        _file = os.path.normpath(_d.get("file")).replace(
            os.path.normpath(self.mDat.userPaths_get()["content"]), ""
        )
        ...
        if _d.get("shots"):
            ...
            if mList is False:
                mList = cgmMeta.cgmObject(
                    name="AnimListNode"
                )  # node.Transform(name="AnimListNode")
    ...
    def getMetaDataFromCurrent(self):
        ...
        data["references"] = [
            os.path.normpath(x).replace(os.path.normpath(self.directory), "")
            for x in mc.file(q=True, r=True)
        ]
    ...
    def refreshMetaData(self):
        ...
        if os.path.normpath(self.versionFile) != os.path.normpath(currentFile):
            mc.confirmDialog(
                title="No. Just no.",
                message="The open file doesn't match the selected file. I refuse to refresh this metaData with the wrong file. It just wouldn't feel right.",
                button=["Cancel", "Sorry"],
            )
            return
    ...
    def buildMenu_tools(self, *args):
        ...
        # Export Menu ...
        _exportMenu = mUI.MelMenuItem(
            self.uiMenu_ToolsMenu, l="Export Sets", subMenu=True
        )
        mUI.MelMenuItem(
            _exportMenu,
            l="Set",
    ...
    def buildMenu_category(self, *args):
        ...
        for i, category in enumerate(self.categoryList):
            ...
            self.categoryMenuItemList.append(
                mUI.MelMenuItem(
                    self.categoryMenu, label=category, c=partial(self.SetCategory, i)
                )
            )
            if i == self.categoryIndex:
                self.categoryMenuItemList[i](e=True, enable=False)
    ...
    def buildMenu_subTypes(self, *args):
        ...
        for i, subType in enumerate(self.subTypes):
            ...
            if subType not in self.l_subTypesBase and not b_extra:
                ...
                b_extra = True
            self.subTypeMenuItemList.append(
                mc.menuItem(
                    label=subType,
                    enable=i != self.subTypeIndex,
                    c=cgmGEN.Callback(self.SetSubType, i),
                )
            )  # mUI.MelMenuItem(self.subTypeMenu, label=subType, c=partial(self.SetSubType,i)) )

        mUI.MelMenuItemDiv(self.subTypeMenu, label="Utils..")
        mUI.MelMenuItem(
            self.subTypeMenu, label="Create Subtype", c=partial(self.CreateSubType)
        )
        try:
            mUI.MelMenuItem(
                self.subTypeMenu,
                label="Rename '{}'".format(self.subTypes[self.subTypeIndex]),
                command=partial(self.rename_below, "subtype"),
            )
        except:
            mUI.MelMenuItem(
                self.subTypeMenu,
    ...
    def build_searchable_list(self, parent=None, sc=None, refreshCommand=None):
        ...
        form(
            ...
            attachControl=[(tsl, "top", _margin, rcl)],
        )
    ...
    def process_search_filter(self, searchableList, *args):
        ...
        if not searchableList["searchField"].getValue():
            ...
            searchTerms = (
                searchableList["searchField"].getValue().lower().strip().split(" ")
            )  # set(searchableList['searchField'].getValue().replace(' ', '').lower())
            listItems = []
    ...
    def clear_search_filter(self, searchableList, *args):
        log.debug(
            "Clearing search filter for %s with search term %s"
            % (searchableList["scrollList"], searchableList["searchField"].getValue())
        )
    ...
    def SetCategory(self, index, *args):
        ...
        try:
            self.l_subTypesBase = [
                x["n"]
                for x in self.mDat.assetType_get(self.category).get(
                    "content", [{"n": "animation"}]
                )
            ]
        except:
        ...
        try:
            self.subTypes = [
                x["n"] for x in self.mDat.assetType_get(self.category)["content"]
            ]
        except:
            self.subTypes = ["None"]
    ...
    def SetSubType(self, index, *args):
        ...
        if not self.b_loadState:
            try:
                self.var_lastSubtype.setValue(self.subTypes[self.subTypeIndex])
            except:
    ...
    def LoadVariationList(self, *args):
        ...
        if (
            self.path_set
        ):  # self.path_dir_category and self.assetList['scrollList'].getSelectedItem() and self.subTypeSearchList['scrollList'].getSelectedItem():
            ...
            if os.path.exists(animationDir):
                for d in CGMOS.get_lsFromPath(
                    animationDir, "dir"
                ):  # os.listdir(animationDir):
                    # for ext in fileExtensions:
                    # 	if os.path.splitext(f)[-1].lower() == ".%s" % ext :
                    if d[0] == "_" or d[0] == ".":
                        continue
                ...
                log.error(
                    log_msg(_str_func, "path doesn't exist? {}".format(animationDir))
                )
                ...
                if self.showAllFiles:
                    ...
                    if self.hasSub:
                        if self.hasVariant:
                            ...
                            if (
                                "{0}_{1}_".format(self.selectedAsset, self.selectedSet)
                                in f
                            ):
                                anims.append(f)
                    else:
        ...
        if anims:
            if selectValue:
                ...
                if _lastVersion and _lastVersion in searchList["scrollList"]._items:
                    searchList["scrollList"].selectByValue(
                        _lastVersion, selCommand=False
                    )
                else:
    ...
    def uiFunc_getOpenFileDict(self, *args):
        ...
        if _content in _current:
            ...
            d_fields = {
                "asset": self.assetList["scrollList"],
            ...
            if self.mDat:  # Adding the ability to load to Scene
                for i, d in enumerate(self.mDat.assetDat):
                    ...
                    if k in _dat["split"]:
                        ...
                        if numItemsFound > 3:
                            self.subTypeSearchList["scrollList"].clearSelection()
                            self.subTypeSearchList["scrollList"].selectByValue(
                                l_temp[3]
                            )
                            self.LoadVariationList()
                        ...
                        if numItemsFound > 4:
                            if self.hasVariant:
                                self.variationList["scrollList"].clearSelection()
                                self.variationList["scrollList"].selectByValue(
                                    l_temp[4]
                                )
                                self.LoadVersionList()
                                if numItemsFound > 5:
                                    self.versionList["scrollList"].selectByValue(
                                        l_temp[5]
                                    )
                            else:
                                self.versionList["scrollList"].selectByValue(l_temp[4])
    ...
    def SaveCurrentSelection(self, *args):
        ...
        if self.variationList["scrollList"].getSelectedItem():
            self.var_lastVariation.setValue(
                self.variationList["scrollList"].getSelectedItem()
            )
        # else:
        ...
        if result == "OK":
            ...
            for subType in self.l_subTypesBase:
                if not os.path.exists(
                    os.path.normpath(os.path.join(charPath, subType))
                ):
    ...
    def DuplicateAssetStructure(self, *args):
        ...
        if result == "OK":
            ...
            _path1 = os.path.normpath(
                os.path.join(self.path_dir_category, _currentChar)
            )
    ...
    def CreateSubType(self, *args):
        ...
        if result == "OK":
            ...
            subTypeDir = (
                self.path_asset
            )  # os.path.normpath(os.path.join(self.path_asset, self.subType)) if self.hasSub else os.path.normpath(self.path_asset)
    ...
    def CreateSubTypeRef(self, *args):
        ...
        wantedName = "{}Ref.{}".format(
            wantedName, self.d_tf["general"]["mayaFilePref"].getValue()
        )
        ...
        if len(existingFiles) == 0:
            ...
            if baseName != wantedBasename:
                ...
                try:
                    versionPrefix = versionString[
                        : versionString.find(versionNumString)
                    ]
                except:
                    versionPrefix = ""
            else:
            ...
            for item in existingFiles:
                ...
                if pattern.match(item):
                    ...
                    versions.append(
                        int(
                            item.split(".")[0].split("_")[-1].replace(versionPrefix, "")
                        )
                    )
    ...
    def LoadProject(self, path, *args):
        ...
        if mDat.d_project.get("mayaVersionCheck"):
            ...
            if _current != _expected:
                ...
                log.warning(
                    "Expected maya version not found. Current: {} | Expected: {}".format(
                        _current, _expected
                    )
                )
        ...
        try:
            ...
            vLite = [MATH.Clamp(1.7 * v, 0.5, 1.0) for v in _bgColor]
        ...
        if self.mDat.d_exportOptions:
            self.var_postEuler.setValue(
                self.mDat.d_exportOptions.get("postEuler", True)
            )
            self.var_postTangent.setValue(
                self.mDat.d_exportOptions.get("postTangent", True)
            )

            if self.mDat.d_exportOptions.get("removeNameSpace"):
                self.var_removeNamespace.setValue(
                    self.mDat.d_exportOptions.get("removeNameSpace", False)
                )
                self.removeNamespace = self.mDat.d_exportOptions.get(
        ...
        if result == "OK":
            ...
            try:
                ...
                return log.warning(
                    log_msg(
                        _str_func,
                        "Error on rename. Check if you have one of the directories open as file browsers",
                    )
                )
            ...
            if mode == "asset":
                ...
                self.assetList["scrollList"].selectByValue(
                    self.var_lastAsset.getValue()
                )
            ...
            if mode in ["variant", "variation"]:
                ...
                self.variationList["scrollList"].selectByValue(
                    self.var_lastVariation.getValue()
                )
    ...
    def RenameAsset(self, *args):
        ...
        if result == "OK":
            ...
            for animation in os.listdir(os.path.join(self.path_asset, self.subType)):
                for variation in os.listdir(
                    os.path.join(self.path_asset, self.subType, animation)
                ):
                    for version in os.listdir(
                        os.path.join(
                            self.path_asset, self.subType, animation, variation
                        )
                    ):
                        if originalAssetName in version:
                            ...
                            newPath = os.path.join(
                                ...
                                animation,
                                variation,
                                version.replace(originalAssetName, newName),
                            )
            ...
            for baseFile in os.listdir(self.path_asset):
                if os.path.isfile(os.path.join(self.path_asset, baseFile)):
                    if originalAssetName in baseFile:
                        ...
                        newPath = os.path.join(
                            self.path_asset,
                            baseFile.replace(originalAssetName, newName),
                        )
                        os.rename(originalPath, newPath)

            # rename folder
            os.rename(
                self.path_asset, self.path_asset.replace(originalAssetName, newName)
            )
    ...
    def OpenSubTypeDirectory(self, *args):
        if self.path_asset:
            self.OpenDirectory(
                os.path.normpath(os.path.join(self.path_asset, self.subType))
            )
        else:
    ...
    def ReferenceFile(self, *args):
        ...
        if self.versionFile and os.path.exists(self.versionFile):
            _namespace = (
                self.assetList["scrollList"].getSelectedItem()
                if self.hasSub
                else self.selectedAsset
            )
            mc.file(
                filePath,
                r=True,
                ignoreVersion=True,
                namespace=CORESTRING.stripInvalidChars(_namespace),
            )
        else:
    ...
    def UpdateAssetTSLPopup(self, *args):
        ...
        renameAssetMB = mUI.MelMenuItem(
            self.assetTSLpum,
            label="Rename Asset",
            command=partial(self.rename_below, "asset"),
        )
        mUI.MelMenuItem(
            self.assetTSLpum,
            label="Duplicate Structure",
            command=self.DuplicateAssetStructure,
            en=1,
        )

        openInExplorerMB = mUI.MelMenuItem(
            self.assetTSLpum, label="Open In Explorer", command=self.OpenAssetDirectory
        )
        openMayaFileHereMB = mUI.MelMenuItem(
            self.assetTSLpum,
            label="Open In Maya",
            command=lambda *a: self.uiPath_mayaOpen(
                os.path.join(self.path_dir_category, self.selectedAsset)
            ),
        )
        ...
        for subType in self.subTypes:
            ...
            try:
                subDir = os.path.join(self.path_asset, subType)
            except:
                continue
            ...
            for i, rig in enumerate(assetList.versions):
                item = mUI.MelMenuItem(
                    openRigMB, l=rig, c=cgmGEN.Callback(self.OpenRig, directoryList[i])
                )
                ...
                item = mUI.MelMenuItem(
                    ...
                    l=rig,
                    c=cgmGEN.Callback(
                        self.ReferenceRig, directoryList[i], self.selectedAsset
                    ),
                )
        ...
        self.refreshAssetListMB = mUI.MelMenuItem(
            self.assetTSLpum, label="Refresh", command=self.LoadCategoryList
        )
        ...
        mUI.MelMenuItem(
            self.assetTSLpum,
            label="Delete",
            command=lambda *a: self.uiFunc_deleteSelectedInList("asset"),
        )
    ...
    def UpdateVersionTSLPopup(self, mMenu=None, *args):
        ...
        for i, p in enumerate(mPathList.mOptionVar.value):
            ...
            if self.mDat.userPaths_get().get("content") == mProj.userPaths_get().get(
                "content"
            ):
                continue
            ...
            item = mUI.MelMenuItem(
                ...
                c=partial(
                    self.SendVersionFileToProject, {"filename": asset, "project": p}
                ),
            )
    ...
    def SendVersionFileToProject(self, infoDict, *args):
        ...
        newFilename = os.path.normpath(_file).replace(
            os.path.normpath(self.mDat.userPaths_get()["content"]),
        ...
        if result == "Yes" or self.var_alwaysSendReferenceFiles.getValue():
            ...
            for refFile in mc.file(_file, query=True, reference=True):
                ...
                newRefFilename = os.path.normpath(refFile).replace(
                    os.path.normpath(self.mDat.userPaths_get()["content"]),
                    os.path.normpath(newProject.userPaths_get()["content"]),
                )
    ...
    def VerifyAssetDirs(self):
        ...
        if not os.path.exists(assetPath):
            ...
            log.info(
                "{0}>> Path not found. Appending: {1}".format(_str_func, assetPath)
            )
        for subType in self.subTypes:
            ...
            if not os.path.exists(subPath):
                ...
                log.info(
                    "{0}>> Path not found. Appending: {1}".format(_str_func, subPath)
                )

    def AddLastToExportQueue(self, *args):
        if self.variationList != None:
            self.batchExportItems.append(
                {
    ...
    def batch_buildFile(self, *args):
        ...
        if self.useMayaPy:
            ...
            l_dat = []
            d_base = {
                "removeNamespace": self.d_tf["exportOptions"][
                    "removeNameSpace"
                ].getValue(),
            ...
            for animDict in self.batchExportItems:
                categoryDirectory = os.path.normpath(
                    os.path.join(self.directory, animDict["category"])
                )
                path_asset = os.path.normpath(
                    os.path.join(categoryDirectory, animDict["asset"])
                )
                ...
                path_set = os.path.normpath(
                    os.path.join(path_asset, animDict["subType"])
                )
                ...
                if animDict.get("path"):
                    ...
                    if animDict.get("variation"):
                        path_variationDirectory = os.path.normpath(
                            os.path.join(path_set, animDict["variation"])
                        )
                    else:
                        path_variationDirectory = path_set

                    versionFile = os.path.normpath(
                        os.path.join(path_variationDirectory, animDict["version"])
                    )

                categoryExportPath = os.path.normpath(
                    os.path.join(self.exportDirectory, animDict["category"])
                )
                exportAssetPath = os.path.normpath(
                    os.path.join(categoryExportPath, animDict["asset"])
                )
                exportAnimPath = os.path.normpath(
                    os.path.join(exportAssetPath, animDict["subType"])
                )
                ...
                d = {
                    ...
                    "updateAndIncrement": int(
                        mc.checkBox(self.updateCB, q=True, v=True)
                    ),
    ...
    def RefreshQueueList(self, *args):
        ...
        for i, item in enumerate(self.batchExportItems):
            self.queueTSL.append(
                "%i ||| Cat: %s | asset: %s | set: %s | var: %s | version: %s | ----- Mode: [ %s ] "
                % (
                    i,
                    item["category"],
                    item["asset"],
    ...
    def RunExportCommand(self, *args):
        ...
        categoryExportPath = os.path.normpath(
            os.path.join(self.exportDirectory, _l_openTokens[0])
        )
        _l_openTokens.pop(0)
        exportAssetPath = os.path.normpath(
            os.path.join(categoryExportPath, _l_openTokens[0])
        )
        _l_openTokens.pop(0)
...
def BatchExport(dataList=[]):
    ...
    for i, fileDat in enumerate(dataList):
        ...
        try:
            _d["categoryExportPath"] = PATHS.NICE_SEPARATOR.join(
                fileDat.get("categoryExportPath")
            )
            _d["exportAnimPath"] = PATHS.NICE_SEPARATOR.join(
                fileDat.get("exportAnimPath")
            )
            _d["exportAssetPath"] = PATHS.NICE_SEPARATOR.join(
                fileDat.get("exportAssetPath")
            )
    ...
    t2 = time.time()
    log.info(
        "|{0}| >> Total Time >> = {1} seconds".format(_str_func, "%0.4f" % (t2 - t1))
    )
...
def ExportScene(
    ...
    d_exportModes = {"export": 1, "cutscene": 2, "rig": 3, "static": 4}
    ...
    if not exportAnimPath:
        ...
        if exportAsRig:
            exportAnimPath = os.path.normpath(
                os.path.join(exportAssetPath, subSet, subType)
            )
        else:
    ...
    if exportAsCutscene:
        ...
        if animationName is not None:
            exportAnimPath = os.path.normpath(
                os.path.join(exportAnimPath, animationName)
            )
    ...
    if not bakeSetName:
        bakeSetName = cgmMeta.cgmOptionVar(
            "cgm_bake_set", varType="string", defaultValue="bake_tdSet"
        ).getValue()
    if not deleteSetName:
        deleteSetName = cgmMeta.cgmOptionVar(
            "cgm_delete_set", varType="string", defaultValue="delete_tdSet"
        ).getValue()
    if not exportSetName:
        exportSetName = cgmMeta.cgmOptionVar(
            "cgm_export_set", varType="string", defaultValue="export_tdSet"
        ).getValue()
    ...
    for obj in exportObjs:
        ...
        if exportStatic:
            exportFile = os.path.normpath(
                os.path.join(exportAssetPath, "{}.fbx".format(cgmObj.p_nameBase))
            )
        else:
        ...
        if exportStatic:
            ...
            if exportFBXFile:
                ...
                for shot in animList.shotList:
                    ...
                    log.info(shot)
                    mel.eval(
                        'FBXExportSplitAnimationIntoTakes -v "{}" {} {}'.format(
...
def PurgeOptionVars():
    optionVarProjectStore = cgmMeta.cgmOptionVar(
        "cgmVar_projectCurrent", varType="string"
    )
    optionVarProjectStore.purge()

    optionVarLastAssetStore = cgmMeta.cgmOptionVar(
        "cgmVar_sceneUI_last_asset", varType="string"
    )
    optionVarLastAssetStore.purge()

    optionVarLastAnimStore = cgmMeta.cgmOptionVar(
        "cgmVar_sceneUI_last_animation", varType="string"
    )
    optionVarLastAnimStore.purge()

    optionVarLastVariationStore = cgmMeta.cgmOptionVar(
        "cgmVar_sceneUI_last_variation", varType="string"
    )
    optionVarLastVariationStore.purge()

    optionVarLastVersionStore = cgmMeta.cgmOptionVar(
        "cgmVar_sceneUI_last_version", varType="string"
    )
    optionVarLastVersionStore.purge()

    showAllFilesStore = cgmMeta.cgmOptionVar(
        "cgmVar_sceneUI_show_all_files", defaultValue=0
    )
    showAllFilesStore.purge()

    removeNamespaceStore = cgmMeta.cgmOptionVar(
        "cgmVar_sceneUI_remove_namespace", defaultValue=0
    )
    ...
    alwaysSendReferenceFiles = cgmMeta.cgmOptionVar(
        "cgmVar_sceneUI_alwaysSendReferences", defaultValue=0
    )
